...
- **대공정 + 세부항목 2단 구조** — 표준 대공정 13종(철거~기타)은 고정하고, 그 아래 기본 세부항목(예: 목공 → 방문 교체·몰딩·가벽) + 커스텀 세부항목으로 견적서를 입력
...
```
Category         대공정 (시드 13개 + 커스텀)
SubItemTemplate  대공정별 기본 세부항목 (예: 목공 → 방문 교체)
Vendor           업체 (부가세 포함 여부 등)
LineItem         업체 × 공정 세부항목 금액 줄 (기본 or 커스텀 이름)
QuoteItem        업체 × 공정 상태: EXCLUDED | BUNDLED | UNKNOWN
